...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr  9 14:01:32 2025
...
# %% Config parameters and load Adot
...
cfg_img_recon = {
    ...
    'alpha_spatial' : 1e-3,    #  spatial reg , small pushes deeper into the brain   -- # use smaller alpha spatial od 10^-2 or -3 w/out cmeas
    'spectrum' : 'prahl',
    'cfg_sb' : cfg_sb,
    ...
    for idx_subj, curr_subj in enumerate(cfg_dataset['subj_ids']):
        ...
        if cfg_img_recon['flag_Cmeas']:
            ...
            if C is None or D is None:
                ...
                X_hrf_mag_tmp, W, C, D = pfDAB_img.do_image_recon_DB( hrf_od = hrf_od_mag, head = head, Adot = Adot, C_meas = C_meas,
                                                                  wavelength = wavelength, cfg_img_recon = cfg_img_recon, 
                                                                  trial_type_img = trial_type, save_path = save_path,
                                                                  W = None, C = None, D = None) 
        
            else:
                X_hrf_mag_tmp, W, _, _ = pfDAB_img.do_image_recon_DB( hrf_od = hrf_od_mag, head = head, Adot = Adot, C_meas = C_meas, 
                                                                  wavelength = wavelength, cfg_img_recon = cfg_img_recon, 
                                                                  trial_type_img = trial_type, save_path = save_path, 
                                                                  W = None, C = C, D = D)
        else:
            cov_str = ''
            if C is None or D is None:
                X_hrf_mag_tmp, W, C, D = pfDAB_img.do_image_recon_DB( hrf_od = hrf_od_mag, head = head, Adot = Adot, C_meas = None,
                                                                  wavelength = wavelength, cfg_img_recon = cfg_img_recon, 
                                                                  trial_type_img = trial_type, save_path = save_path,
                                                                  W = None, C = None, D = None) 
        
            else:
                X_hrf_mag_tmp, W, _, _ = pfDAB_img.do_image_recon_DB( hrf_od = hrf_od_mag, head = head, Adot = Adot, C_meas = None, 
                                                                  wavelength = wavelength, cfg_img_recon = cfg_img_recon, 
                                                                  trial_type_img = trial_type, save_path = save_path, 
                                                                  W = None, C = C, D = D)
...
if not cfg_img_recon['SB']:
    filepath = os.path.join(save_path, f'Xs_{tasknm}_direct_alltrial_{cov_str}_alpha_spatial_{cfg_img_recon["alpha_spatial"]:.0e}_alpha_meas_{cfg_img_recon["alpha_meas"]:.0e}.pkl.gz')
    print(f'   Saving to Xs_{tasknm}_direct_alltrials_{cov_str}_alpha_spatial_{cfg_img_recon["alpha_spatial"]:.0e}_alpha_meas_{cfg_img_recon["alpha_meas"]:.0e}.pkl.gz')
    file = gzip.GzipFile(filepath, 'wb')
    file.write(pickle.dumps(results_img_s))
    file.close()     
else:
    filepath = os.path.join(save_path, f'Xs_{tasknm}_direct_alltrial_{cov_str}_alpha_spatial_{cfg_img_recon["alpha_spatial"]:.0e}_alpha_meas_{cfg_img_recon["alpha_meas"]:.0e}_SB_sigma_brain_{cfg_img_recon["sigma_brain"]}_sigma_scalp_{cfg_img_recon["sigma_scalp"]}.pkl.gz')
    print(f'   Saving to Xs_{tasknm}_direct_alltrials_{cov_str}_alpha_spatial_{cfg_img_recon["alpha_spatial"]:.0e}_alpha_meas_{cfg_img_recon["alpha_meas"]:.0e}.pkl.gz')
    file = gzip.GzipFile(filepath, 'wb')
    file.write(pickle.dumps(results_img_s))
    file.close()     
